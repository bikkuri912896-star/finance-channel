...
def _font_latin(size: int, bold: bool = True) -> ImageFont.FreeTypeFont:
    path = config.FONT_PATH_LATIN if bold else config.FONT_PATH_LATIN_REGULAR
    try:
        return ImageFont.truetype(path, size)
    except Exception:
...
def _gold_line(draw, y: int, width: int = 700, color=None):
    c = color or config.COLOR_GOLD
    x0 = (W - width) // 2
    draw.rectangle([x0, y, x0 + width, y + 2], fill=c)
...
def _load_bg(image_path: Path) -> np.ndarray:
    ...
    return np.array(Image.blend(img, overlay, 0.55))
...
def _intro_clip(scene: dict, dur: float) -> VideoClip:
    ...
    def frame(t):
        ...
        _gold_line(draw, 160, color=gold)
        _draw_centered(draw, config.CHANNEL_NAME, 270, f_ch, gold)
        _gold_line(draw, 328, color=gold)
        _draw_centered(draw, config.CHANNEL_TAGLINE, 410, f_tg, cream, shadow=False, wrap=22)
...
def _word_clip(scene: dict, dur: float) -> VideoClip:
    # Japanese financial term — use JP font (not Times New Roman)
    f_term = _font_jp(120, bold=True)
    f_en   = _font_latin(44, bold=False)
    f_cat  = _font_jp(38)
    ...
    def frame(t):
        ...
        gold  = tuple(int(c * fade) for c in config.COLOR_GOLD)
        cream = tuple(int(c * fade) for c in config.COLOR_CREAM)
        accent = tuple(int(c * fade) for c in config.COLOR_ACCENT)

        _gold_line(draw, H // 2 - 270, color=gold)

        term = _nfc(scene.get("latin_text", ""))
        # Use shorter font for long terms
        f_display = _font_jp(80, bold=True) if len(term) > 6 else f_term
        _draw_centered(draw, term, H // 2 - 60, f_display, gold, wrap=10)

        eng = _nfc(scene.get("romanji", ""))
        if eng:
            _draw_centered(draw, eng, H // 2 + 100, f_en, cream, shadow=False)

        cat = scene.get("reading", "")
        if cat:
            _draw_centered(draw, cat, H // 2 + 185, f_cat, accent, shadow=False)

        _gold_line(draw, H // 2 + 240, color=gold)
...
def _meaning_clip(scene: dict, image_path, dur: float) -> VideoClip:
    ...
    def frame(t):
        ...
        if subtitle:
            _gold_line(draw, 120, color=gold)
            _draw_centered(draw, subtitle, 210, f_sub, gold, wrap=14)
            _gold_line(draw, 268, color=gold)
        quote = scene.get("latin_quote", "")
        if quote:
            _draw_centered(draw, _nfc(f"— {quote} —"),
                           H // 2 - 50, f_qt, gold, wrap=22)
